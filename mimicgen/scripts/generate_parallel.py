...
def generate_worker(env_kwargs, env_interface_kwargs, data_generator_kwargs, **kwargs):
    if not hasattr(generate_worker, 'initialized'):
        print(f"Process {os.getpid()} - initializing environment...")
        generate_worker.env = RobomimicUtils.create_env(**env_kwargs)
        generate_worker.env_interface = make_interface(
            **env_interface_kwargs,
            # NOTE: env_interface takes underlying simulation environment, not robomimic wrapper
            env=generate_worker.env.base_env,
        )
        generate_worker.data_generator = DataGenerator(
            **data_generator_kwargs
        )
        generate_worker.initialized = True
        
    traj = generate_worker.data_generator.generate(
        env=generate_worker.env,
        env_interface=generate_worker.env_interface,
        **kwargs
    )
...
def generate_dataset(
    ...
    env = RobomimicUtils.create_env(**env_kwargs)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--workers",
        type=int,
        help="the number of parallel jobs to run (by default will use all CPU cores)",
        default=-1,
    )
