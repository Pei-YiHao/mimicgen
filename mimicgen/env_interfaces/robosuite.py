...
class MG_StackFour(RobosuiteInterface):
    """
    Corresponds to robosuite StackThree task and variants.
    """
    def get_object_poses(self):
        ...
        return dict(
            ...
            cubeB=self.get_object_pose(obj_name=self.env.cubeB.root_body, obj_type="body"),
            cubeC=self.get_object_pose(obj_name=self.env.cubeC.root_body, obj_type="body"),
            cubeD=self.get_object_pose(obj_name=self.env.cubeD.root_body, obj_type="body"),
        )
    ...
    def get_subtask_term_signals(self):
        ...
        # fourth subtask is placing cubeC on cubeA (motion relative to cubeA)
        signals["stack_2"] = int(self.env._check_cubeC_stacked())
        
        # fifth subtask is grasping cubeD (motion relative to cubeD)
        signals["grasp_3"] = int(self.env._check_grasp(gripper=self.env.robots[0].gripper, object_geoms=self.env.cubeD))
